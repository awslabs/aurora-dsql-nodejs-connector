 ...
 * Source: https://github.com/porsager/postgres/blob/5c8135f3df1bb10e7aad10f14a6f084db3724f82/tests/index.js
 */

/* Notes: 
 * t() - enabled test cases 
 * nt() - disabled test cases 
 * ot() - test only (multiple ot() test cases are allowed)
 * when using ot() - be aware that some tests requires connection to be established in previous
 * test cases. When running a particular test case (such as reserve connection), it might timeout due to  
...
import { auroraDSQLWsPostgres as postgres } from '../../dist/index.mjs'
...
nt('Uses default database without slash', async () => {
  const sql = postgres('postgres://localhost')
  return [sql.options.user, sql.options.database]
})

nt('Uses default database with slash', async () => {
  const sql = postgres('postgres://localhost/')
  return [sql.options.user, sql.options.database]
})
...
t('Create table', async () =>
  ['CREATE TABLE', (await sql`create table if not exists test(int int)`).command, await sql`drop table test`]
)

t('Drop table', { timeout: 30 }, async () => {
  await sql`create table if not exists test(int int)`
...
nt('Transaction throws on uncaught savepoint', async () => {
  await sql`create table test (a int)`
...
nt('Transaction throws on uncaught named savepoint', async () => {
  await sql`create table test (a int)`
...
nt('Transaction succeeds on caught savepoint', async () => {
  await sql`create table test (a int)`
  await sql.begin(async sql => {
    await sql`insert into test values(1)`
    await sql.savepoint(async sql => {
...
nt('Savepoint returns Result', async () => {
  let result
  await sql.begin(async sql => {
...
nt('Transaction requests are executed implicitly', async () => {
...
nt('Connect using uri', async () =>
  [true, await new Promise((resolve, reject) => {
    const sql = postgres('postgres://' + login.user + ':' + (login.pass || '') + '@' + options.hostname + ':5432/' + options.db, {
      idle_timeout
    })
...
nt('Options from uri with special characters in user and pass', async () => {
...
nt('Fail with proper error on no host', async () =>
  ['ECONNREFUSED', (await new Promise((resolve, reject) => {
    const sql = postgres('postgres://' + options.hostname + ':33333/' + options.db, {
      idle_timeout
    })
...
nt('Connect using SSL direct', async () => {
...
nt('Connect using SSL prefer', async () => {
...
nt('Proper handling of non object Errors', async () => {
...
nt('Proper handling of null Errors', async () => {
...
nt('Ensure reserve on connection throws proper error', async () => {
...
nt('Support dynamic password function', async () => {
...
nt('Support dynamic async password function', async () => {
  return [true, (await postgres({
    ...options,
    ...login_scram,
...
nt('Transform nested json in arrays', async () => {
  const sql = postgres({
    ...options,
...
nt('Transform deeply nested json object in arrays', async () => {
  const sql = postgres({
    ...options,
    transform: postgres.camel
  })
...
nt('Transform deeply nested json array in arrays', async () => {
  const sql = postgres({
    ...options,
    transform: postgres.camel
  })
...
nt('Bypass transform for json primitive', async () => {
  const sql = postgres({
    ...options,
    transform: postgres.camel
  })
...
nt('Bypass transform for jsonb primitive', async () => {
  const sql = postgres({
    ...options,
    transform: postgres.camel
  })
...
nt('multiple listeners work after a reconnect', async () => {
  const sql = postgres(options)
    , xs = []
...
nt('listen and notify with weird name', async () => {
  const sql = postgres(options)
  const channel = 'wat-;.ø.§'
  const result = await new Promise(async r => {
...
nt('listen and notify with upper case', async () => {
  const sql = postgres(options)
...
nt('listen reconnects', { timeout: 30 }, async () => {
  const sql = postgres(options)
    , resolvers = {}
...
nt('listen result reports correct connection state after reconnection', async () => {
  const sql = postgres(options)
    , xs = []
...
nt('unlisten removes subscription', async () => {
  const sql = postgres(options)
    , xs = []
...
nt('listen after unlisten', async () => {
  const sql = postgres(options)
    , xs = []
...
nt('multiple listeners and unlisten one', async () => {
  const sql = postgres(options)
    , xs = []
...
nt('responds with server parameters (application_name)', async () =>
...
nt('has server parameters', async () => {
  return ['postgres.js', (await sql`select 1`.then(() => sql.parameters.application_name))]
})

nt('big query body', { timeout: 30 }, async () => {
  await sql`create table test (x int)`
  return [50000, (await sql`insert into test ${sql([...Array(50000).keys()].map(x => ({ x })))
    }`).count, await sql`drop table test`]
})

nt('Throws if more than 65534 parameters', async () => {
  await sql`create table test (x int)`
  return ['MAX_PARAMETERS_EXCEEDED', (await sql`insert into test ${sql([...Array(65535).keys()].map(x => ({ x })))
...
nt('connection parameters', async () => {
  const sql = postgres({
    ...options,
    connection: {
      'some.var': 'yay'
...
t('Multiple Cursors', { timeout: 30 }, async () => {
  const result = []
  await sql.begin(async sql => [
...
t('Transform columns from', async () => {
  ...
  return [
    2,
    (await sql`select ${sql('a_test', 'a_test')} from test`)[0].a_test,
    await sql`drop table test`
  ]
})
...
t('Transform columns to', async () => {
  ...
  return [
    2,
    (await sql`select a_test, b_test from test`)[0].a_test,
    await sql`drop table test`
  ]
})
...
t('Transform columns from and to', async () => {
  ...
  return [
    2,
    (await sql`select ${sql('a_test', 'b_test')} from test`)[0].a_test,
    await sql`drop table test`
  ]
})
...
nt('connect_timeout', { timeout: 20 }, async () => {
...
nt('connect_timeout throws proper error', async () => [
  'CONNECT_TIMEOUT',
  await postgres({
    ...options,
    ...login_scram,
...
nt('connect_timeout error message includes host:port', { timeout: 20 }, async () => {
  const connect_timeout = 0.2
  const server = net.createServer()
  server.listen()
  const sql = postgres({ port: server.address().port, host: '127.0.0.1', connect_timeout })
...
nt('Recreate prepared statements on transformAssignedExpr error', { timeout: 1 }, async () => {
...
nt('Recreate prepared statements on RevalidateCachedQuery error', async () => {
...
nt('Properly throws routine error on not prepared statements', async () => {
...
nt('Properly throws routine error on not prepared statements in transaction', async () => {
...
nt('Properly throws routine error on not prepared statements using file', async () => {
  const { routine } = await sql.unsafe(`
...
t('Escaping supports schemas and tables', async () => {
  ...
  return [
    ...
    await sql`drop schema a cascade`
  ]
})
...
t('Copy from file', async () => {
  ...
  return [
    JSON.stringify(await sql`select * from test order by x`),
...
nt('subscribe with transform', { timeout: 30 }, async () => {
  const sql = postgres({
    transform: {
      column: {
        from: postgres.toCamel,
...
nt('subscribe reconnects and calls onsubscribe', { timeout: 4 }, async () => {
  const sql = postgres({
    database: 'postgres_js_test',
    publications: 'alltables',
    fetch_types: false
...
nt('Cancel piped query', { timeout: 20 }, async () => {
  await sql`select 1`
...
t('Ensure reconnect after max_lifetime with transactions', { timeout: 30 }, async () => {
  const sql = postgres({
    ...options,
    max_lifetime: 0.01,
    idle_timeout,
...
nt('Supports fragments as dynamic parameters', async () => {
...
nt('Does not try rollback when commit errors', async () => {
  let notice = null
...
nt('Insert array with null', async () => {
  await sql`create table test (x int[])`
...
nt('Insert array with undefined throws', async () => {
  await sql`create table test (x int[])`
  return [
    'UNDEFINED_VALUE',
...
nt('Insert array with undefined transform', async () => {
  const sql = postgres({ ...options, transform: { undefined: null } })
  await sql`create table test (x int[])`
...
t('Ensure reserve on query throws proper error', async () => {
  const sql = postgres({ ...options, idle_timeout }) // eslint-disable-line
  const reserved = await sql.reserve()
...
nt('query during copy error', async () => {
